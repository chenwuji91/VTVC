...
if __name__ == "__main__":
    ...
    markers = 'p'
    linestyles = ['dashed']
    method, yerr = readFile('FIG8_TIMES.csv')
    x = range(1, 10)
    fig, ax = plt.subplots(figsize=(8, 6))
    lines = []
#    for i in range(len(method)):
    for i in range(len(method)):
        l, e, b = ax.errorbar(x, method[i], elinewidth=2.3, linewidth=4, marker=markers[i], markersize=8, color='navy', yerr=yerr[i], ls=linestyles[i])
        lines.append(l)
#    lines.append(l)
    fontsize = 14
    ...
    ax.set_xlim(0, 10)
    plt.yticks(np.arange(0., 0.7, 0.1), weight='bold')
