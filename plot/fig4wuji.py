...
if __name__ == "__main__":
    ...
    markers = ['p', 'D', 'o']
    linestyles = ['dashed', '-', '--']
    method, yerr = readFile('fig8-4-1.csv')
    x = range(1, 10)
    ...
    ax.set_xlim(0, 10)
    plt.yticks(np.arange(0., 0.61, 0.1), weight='bold')
    #ax.set_yticks(np.arange(0., 0.8, 0.1))

    pr = mf.FontProperties(weight='bold', size=13.5)
    ax.legend(lines, labels, prop=pr, bbox_to_anchor=(0.999, 0.11), numpoints=2, columnspacing=0.8, ncol=5)
